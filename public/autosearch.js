...
pickupInput.onkeyup = function() {
 ...
 dropoffInput.onkeyup = function() {
    ...
    display(result, 'dropoff');
};

function display(result, type) {
    const content = result.map(({name}) => {
        return `<li onclick="selectInput(this, '${type}')">${name}</li>`;
    }).join("");

    if (type === 'pickup') {
        pickupResultBox.innerHTML = `<ul>${content}</ul>`;
    } else if (type === 'dropoff') {
        dropoffResultBox.innerHTML = `<ul>${content}</ul>`;
    }
}

function selectInput(list, type) {
    if (type === 'pickup') {
        pickupInput.value = list.innerHTML;
        pickupResultBox.innerHTML = '';
    } else if (type === 'dropoff') {
        dropoffInput.value = list.innerHTML;
        dropoffResultBox.innerHTML = '';
    }
}
